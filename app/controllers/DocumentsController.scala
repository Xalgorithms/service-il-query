...
@Singleton
class DocumentsController @Inject()(cc: ControllerComponents) extends AbstractController(cc) {
  ...
  def index = Action.async {
    _mongo.find_many(MongoActions.FindManyDocuments()).map { docs =>
      Ok(Json.toJson(docs))
    }
  }

  def show(id: String) = Action.async { req =>
    _mongo.find_one(MongoActions.FindDocumentById(id)).map { doc =>
      Ok(Json.toJson(doc))
    }
  }
}
